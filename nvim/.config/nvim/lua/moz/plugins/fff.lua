return {
	...
	config = function()
		require("fff").setup({
			...
			git = {
				status_text_color = false, -- Enable git status colors on filename text
			},
			grep = {
			...
			hl = {
				...
				normal = "Normal",
				cursor = "CursorLine",
				matched = "GitGraphAuthor",
				title = "GitGraphBranch3",
				prompt = "Question", -- Unused
				active_file = "FFFActive",
				frecency = "FFFGitModified",
				debug = "Comment",
				-- Git Stuff
		...
		},
		{
			...
			function()
				require("fff").find_files_in_dir(
					"~/Library/CloudStorage/ProtonDrive-max@maxhunter.dev-folder/dev/Notes/"
